...
In this case, what you need to do is following a two steps procedure:
...
Proceed to step 2 only if your new node is no longer catching up.
...
In order to properly migrate our validator node to another server we will need to backup the following data(s):
...
3. If you keep your keys on the node make sure you have the mnemonic phrase associated with your key(s).
...
### Migrating the data in to the fullnode
With both nodes stopped we will be copying the backedup data in to the newly synced fullnode.

:::warning Warning
At this point, both nodes should not be running. This is to prevent any possible side effects such as double signing
:::

Transfer the following backed up files from the old node into the new one:
...
Once you have moved your `priv_validator_key.json` and `priv_validator_state` into the newly synced fullnode it will be recognized as a validator node.
...
If you originally had your key in the previous server you can easily add it back using the mnemonic phrase you had backed up
