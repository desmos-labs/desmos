...
test: test-unit
test-all: test-unit test-ledger-mock test-race test-cover test-app2
test-app2: test-app2

TEST_PACKAGES=./...
TEST_TARGETS := test-unit test-unit-amino test-unit-proto test-ledger-mock test-race test-ledger test-race test-app2

# Test runs-specific rules. To add a new test target, just add
# a new rule, customise ARGS or TEST_PACKAGES ad libitum, and
# append the new rule to the TEST_TARGETS list.
test-unit: test_tags += cgo ledger test_ledger_mock norace
test-app2: test_tags += cgo ledger test_ledger_mock norace app_v2
...
.PHONY: all build-linux install \
	...
	test test-all test-cover test-unit test-race test-app2
