	...
	tests := []struct {
		...
		},
		{
			...
			newPost: types2.Post{
				...
				LastEdited: suite.testData.post.LastEdited,
				Subspace:   "4e188d9c17150037d5199bbdb91ae1eb2a78a15aca04cb35530cccb81494b36e",
				AdditionalAttributes: []types2.Attribute{
					types2.NewAttribute("key", "value"),
				},
				Creator: suite.testData.postOwner,
			},
		},
	...
	for _, test := range tests {
		...
		suite.Run(test.name, func() {
			...
			for _, post := range test.existingPosts {
				store.Set(types2.PostStoreKey(post.PostID), suite.cdc.MustMarshalBinaryBare(&post))
			}
			...
			var expected types2.Post
			suite.cdc.MustUnmarshalBinaryBare(store.Get(types2.PostStoreKey(test.newPost.PostID)), &expected)
			suite.True(expected.Equal(test.newPost))

			// Check the parent comments
			var wrapped types2.CommentIDs
			suite.cdc.MustUnmarshalBinaryBare(store.Get(types2.PostCommentsStoreKey(test.newPost.ParentID)), &wrapped)
...
func (suite *KeeperTestSuite) TestKeeper_GetPost() {
	...
	for _, test := range tests {
		...
		suite.Run(test.name, func() {
			...
			if test.postExists {
				store.Set(types2.PostStoreKey(test.expected.PostID), suite.cdc.MustMarshalBinaryBare(&test.expected))
			}
...
func (suite *KeeperTestSuite) TestKeeper_GetPostsFiltered() {
	...
	tests := []struct {
		name     string
		filter   types2.QueryPostsParams
		expected []types2.Post
	}{
		{
			name:     "Valid pagination works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 2},
			expected: []types2.Post{posts[1], posts[2]},
		},
		{
			name:     "Non existing page returns empty list",
			filter:   types2.QueryPostsParams{Page: 10, Limit: 1},
			expected: []types2.Post{},
		},
		{
			name:     "Invalid pagination returns all data",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 0},
			expected: []types2.Post{posts[1], posts[2], posts[0]},
		},
		{
			name:     "Parent ID matcher works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, ParentID: posts[0].ParentID},
			expected: []types2.Post{posts[1], posts[0]},
		},
		{
			name:     "Creation time matcher works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, CreationTime: &date},
			expected: []types2.Post{posts[0]},
		},
		{
			name:     "Subspace mather works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, Subspace: "4e188d9c17150037d5199bbdb91ae1eb2a78a15aca04cb35530cccb81494b36e"},
			expected: []types2.Post{posts[1], posts[2]},
		},
		{
			name:     "Creator mather works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, Creator: "cosmos1jlhazemxvu0zn9y77j6afwmpf60zveqw5480l2"},
			expected: []types2.Post{posts[1], posts[2]},
		},
		{
			name:     "Sorting by date ascending works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, SortBy: types2.PostSortByCreationDate, SortOrder: types2.PostSortOrderAscending},
			expected: []types2.Post{posts[0], posts[1], posts[2]},
		},
		{
			name:     "Sorting by date descending works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, SortBy: types2.PostSortByCreationDate, SortOrder: types2.PostSortOrderDescending},
			expected: []types2.Post{posts[2], posts[1], posts[0]},
		},
		{
			name:     "Sorting by ID ascending works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, SortBy: types2.PostSortByID, SortOrder: types2.PostSortOrderAscending},
			expected: []types2.Post{posts[1], posts[2], posts[0]},
		},
		{
			name:     "Sorting by ID descending works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, SortBy: types2.PostSortByID, SortOrder: types2.PostSortOrderDescending},
			expected: []types2.Post{posts[0], posts[2], posts[1]},
		},
		{
			name:     "Filtering by hashtags works properly",
			filter:   types2.QueryPostsParams{Page: 1, Limit: 5, Hashtags: []string{"desmos", "test"}},
			expected: []types2.Post{posts[0]},
		},
	}
