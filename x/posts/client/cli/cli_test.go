...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	var postsData types2.GenesisState
	s.Require().NoError(cfg.Codec.UnmarshalJSON(genesisState[types2.ModuleName], &postsData))
	...
	postsData.RegisteredReactions = []types2.RegisteredReaction{
	...
	postsData.UsersPollAnswers = []types2.UserAnswer{
	...
	postsData.PostsReactions = []types2.PostReactionsEntry{
	...
	postsData.Reports = []types2.Report{
		types2.NewReport(
	...
	postsData.Params = types2.DefaultParams()

	postsDataBz, err := cfg.Codec.MarshalJSON(&postsData)
	s.Require().NoError(err)
	genesisState[types2.ModuleName] = postsDataBz
...
func (s *IntegrationTestSuite) TestCmdQueryPost() {
	...
	testCases := []struct {
		...
		args           []string
		expectErr      bool
		expectedOutput types2.QueryPostResponse
	}{
		{
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expectErr {
				...
				var response types2.QueryPostResponse
...
func (s *IntegrationTestSuite) TestCmdQueryPosts() {
	...
	testCases := []struct {
		...
		args           []string
		expectErr      bool
		expectedOutput types2.QueryPostsResponse
	}{
		{
			...
			expectErr: false,
			expectedOutput: types2.QueryPostsResponse{
				Posts: []types2.QueryPostResponse{
					{
						Post: types2.Post{
							PostID:               "19de02e105c68a60e45c289bff19fde745bca9c63c38f2095b59e8e8090ae1af",
							Message:              "Post message",
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expectErr {
				...
				var response types2.QueryPostsResponse
				s.Require().NoError(clientCtx.JSONMarshaler.UnmarshalJSON(out.Bytes(), &response), out.String())
...
func (s *IntegrationTestSuite) TestCmdQueryUserAnswers() {
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			cmd := cli2.GetCmdQueryUserAnswers()
			clientCtx := val.ClientCtx
			out, err := clitestutil.ExecTestCLICmd(clientCtx, cmd, tc.args)
			...
			if tc.shouldErr {
				...
				var response types2.QueryUserAnswersResponse
				s.Require().NoError(clientCtx.JSONMarshaler.UnmarshalJSON(out.Bytes(), &response), out.String())
...
func (s *IntegrationTestSuite) TestCmdQueryRegisteredReactions() {
	...
	testCases := []struct {
		...
		args           []string
		expectErr      bool
		expectedOutput types2.QueryRegisteredReactionsResponse
	}{
		{
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			cmd := cli2.GetCmdQueryRegisteredReactions()
			clientCtx := val.ClientCtx
			out, err := clitestutil.ExecTestCLICmd(clientCtx, cmd, tc.args)
			...
			if tc.expectErr {
				...
				var response types2.QueryRegisteredReactionsResponse
				s.Require().NoError(clientCtx.JSONMarshaler.UnmarshalJSON(out.Bytes(), &response), out.String())
...
func (s *IntegrationTestSuite) TestCmdQueryReports() {
	...
	testCases := []struct {
		...
		args           []string
		expectErr      bool
		expectedOutput types2.QueryReportsResponse
	}{
		{
			...
			expectErr:      false,
			expectedOutput: types2.QueryReportsResponse{Reports: []types2.Report{}},
		},
		{
			name: "valid post id",
			args: []string{
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expectErr {
				...
				var response types2.QueryReportsResponse
				s.Require().NoError(clientCtx.JSONMarshaler.UnmarshalJSON(out.Bytes(), &response), out.String())
				s.Require().Equal(tc.expectedOutput, response)
			}
		})
...
func (s *IntegrationTestSuite) TestCmdQueryParams() {
	...
	testCases := []struct {
		...
		args           []string
		expectErr      bool
		expectedOutput types2.QueryParamsResponse
	}{
		{
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expectErr {
				...
				var response types2.QueryParamsResponse
				s.Require().NoError(clientCtx.JSONMarshaler.UnmarshalJSON(out.Bytes(), &response), out.String())
				s.Require().Equal(tc.expectedOutput, response)
			}
		})
...
func (s *IntegrationTestSuite) TestCmdCreatePost() {
	...
	testCases := []struct {
		...
		},
		{
			...
			args: []string{
				"9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08",
				fmt.Sprintf("--%s=%s", cli2.FlagParentID, "parent_id"),
			},
			expErr: true,
		},
		{
			...
			args: []string{
				"9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08",
				"my post",
				fmt.Sprintf("--%s=%s", cli2.FlagCommentsState, "comment_state"),
			},
			expErr: true,
		},
		{
...
func (s *IntegrationTestSuite) TestCmdAddPostReaction() {
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			cmd := cli2.GetCmdAddPostReaction()
			clientCtx := val.ClientCtx
...
func (s *IntegrationTestSuite) TestCmdRemovePostReaction() {
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			cmd := cli2.GetCmdRemovePostReaction()
			clientCtx := val.ClientCtx
...
func (s *IntegrationTestSuite) TestCmdRegisterReaction() {
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			cmd := cli2.GetCmdRegisterReaction()
			clientCtx := val.ClientCtx
