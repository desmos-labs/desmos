...
func createPostHandler(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !parentID.Valid() {
			rest.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("invalid parent ID: %s", parentID))
			return
		}
...
func addReactionToPostHandler(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !postID.Valid() {
			rest.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("invalid parent ID: %s", postID))
			return
		}
