...
func TestReaction_Validate(t *testing.T) {
	tests := []struct {
		name     string
		reaction types2.RegisteredReaction
		error    error
	}{
		{
			name: "Valid reaction returns no error (url on value)",
			reaction: types2.NewRegisteredReaction(
...
func TestPostReactions_ContainsOwnerLike(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			reactions: types2.NewPostReactions(
...
func TestPostReactions_RemoveReaction(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			owner:     "cosmos1s3nh6tafl4amaxkke9kdejhp09lk93g9ev39r4",
			shortcode: ":smile:",
			expResult: types2.NewPostReactions([]types2.PostReaction{}...),
			expEdited: true,
		},
		{
			...
			owner:     "cosmos1s3nh6tafl4amaxkke9kdejhp09lk93g9ev39r4",
			shortcode: ":smile:",
			expResult: types2.NewPostReactions(),
			expEdited: false,
		},
		{
