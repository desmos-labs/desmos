...
// IteratePostReportsByPostID iterates through the post's reports with the given id and performs the provided function
func (k Keeper) IteratePostReportsByPostID(ctx sdk.Context, postID string, fn func(index int64, report types.Report) (stop bool)) {
	store := ctx.KVStore(k.storeKey)
...
// IterateReports iterates through all reports saved inside the given context and perform the given function
