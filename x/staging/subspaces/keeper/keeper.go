...
func (k Keeper) RegisterUserInSubspace(ctx sdk.Context, subspaceID, user, admin string) error {
	...
	// Delete the unregistered key if it exists
	unregisteredKey := types.SubspaceUnregisteredUserKey(subspaceID, user)
...
func (k Keeper) UnregisterUserFromSubspace(ctx sdk.Context, subspaceID, user, admin string) error {
	...
	store.Set(types.SubspaceUnregisteredUserKey(subspaceID, user), []byte(subspaceID))
	return nil
}
