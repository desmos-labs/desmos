	...
	testCases := []struct {
	...
	}{
		{
			...
			store: func(ctx sdk.Context) {
				...
				suite.Require().NoError(suite.pk.StoreProfile(ctx, testutil.ProfileFromAddr(block.Blocked)))
		...
		},
		{
			...
			store: func(ctx sdk.Context) {
				...
				suite.Require().NoError(suite.pk.StoreProfile(ctx, testutil.ProfileFromAddr("cosmos1cjf97gpzwmaf30pzvaargfgr884mpp5ak8f7ns")))
			},
			msg: types.NewMsgBlockUser(
				"cosmos1y54exmx84cqtasvjnskf9f63djuuj68p7hqf47",
				"cosmos1cjf97gpzwmaf30pzvaargfgr884mpp5ak8f7ns",
