...
func GetCmdSaveProfile() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`
Save a new profile or edit the existing one specifying a DTag, a username, biography, profile picture and cover picture.
...
`, version.AppName, FlagUsername, FlagBio, FlagProfilePic, FlagCoverPic),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgSaveProfile(dTag, username, bio, profilePic, coverPic, clientCtx.FromAddress.String())
	...
	cmd.Flags().String(FlagUsername, types.DoNotModify, "Username to be used")
