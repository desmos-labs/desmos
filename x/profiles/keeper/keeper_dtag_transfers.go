...
func (k Keeper) SaveDTagTransferRequest(ctx sdk.Context, request types.DTagTransferRequest) error {
	...
	if store.Has(key) {
		return sdkerrors.Wrapf(types.ErrInvalidDTagTransferRequest,
...
func (k Keeper) DeleteDTagTransferRequest(ctx sdk.Context, sender, recipient string) error {
	...
	if !store.Has(key) {
		return sdkerrors.Wrapf(types.ErrInvalidDTagTransferRequest, "request from %s to %s not found", sender, recipient)
	}
