...
func (k Keeper) OnRecvLinkChainAccountPacket(
	...
	err = k.cdc.UnpackAny(data.DestinationProof.GetPubKey(), &pubKey)
	if err != nil {
	...
	err = data.DestinationProof.Verify(k.cdc)
	if err != nil {
		return packetAck, err
	}

	// Store the link
	chainLink := types.NewChainLink(srcAddrData, data.SourceProof, data.SourceChainConfig, ctx.BlockTime())
	err = k.StoreChainLink(ctx, data.DestinationAddress, chainLink)
	if err != nil {
		return packetAck, err
	}
