...
func randomEditSubspaceFields(
	...
	// Get an editor
	editors, _ := k.GetUsersWithRootPermission(ctx, subspace.ID, types.NewPermissions(types.PermissionEditSubspace))
...
func randomDeleteSubspaceFields(
	...
	// Get an editor
	editors, _ := k.GetUsersWithRootPermission(ctx, subspace.ID, types.NewPermissions(types.PermissionDeleteSubspace))
	acc := GetAccount(RandomAddress(r, editors), accs)
	if acc == nil {
		// Skip the operation without error as the account is not valid
		skip = true
