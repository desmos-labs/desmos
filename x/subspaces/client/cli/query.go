...
func GetCmdQueryUserGroups() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s query subspaces user-groups 1 --page=2 --limit=100`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
...
func GetCmdQueryUserGroupMembers() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s query subspaces user-group-members 1 1 --page=2 --limit=100`, version.AppName),
