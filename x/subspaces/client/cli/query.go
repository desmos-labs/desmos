...
func GetCmdQueryUserAllowances() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "user-fee-allowance [subspace-id] [[grantee]] [[granter]]",
		Short:   "Query allowances of the given user",
		Example: fmt.Sprintf(`%s query subspaces user-fee-allowance 1 desmos1evj20rymftvecmgn8t0xv700wkjlgucwfy4f0c desmos13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud`, version.AppName),
...
func GetCmdQueryGroupAllowances() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "group-fee-allowance [subspace-id] [[group-id]] [[granter]]",
		Short:   "Query allowances of the given user",
		Example: fmt.Sprintf(`%s query subspaces user-fee-allowance 1 1 desmos1evj20rymftvecmgn8t0xv700wkjlgucwfy4f0c`, version.AppName),
		Args:    cobra.RangeArgs(1, 3),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
